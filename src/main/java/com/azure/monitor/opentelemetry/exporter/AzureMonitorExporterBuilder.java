...
public final class AzureMonitorExporterBuilder {
  ...
  public SpanExporter buildTraceExporterWithLiveMetrics() {
    SpanDataMapper mapper =
            new SpanDataMapper(true, this::populateDefaults, (event, instrumentationName) -> false);
    QuickPulse quickPulse = buildQuickPulse();
    return new AzureMonitorTraceExporter(mapper, initExporterBuilder(), quickPulse);
  }

  private QuickPulse buildQuickPulse() {
    Supplier<String> instrumentationKey = () -> connectionString.getInstrumentationKey();
    String roleName = null;
    //String roleInstance = System.getenv("HOSTNAME");
    String roleInstance = null;
    boolean useNormalizedValueForNonNormalizedCpuPercentage = true;
    String agentVersion = "";
    AadAuthentication authentication = new AadAuthentication();

    return QuickPulse.create(
